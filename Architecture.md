...
8. `ApiResponseAdvice` wraps JSON/string controller responses into `Response<T>` unless excluded. The `Response` envelope is annotated `@JsonInclude(NON_NULL)` and `AtlasWebAutoConfiguration` contributes a Jackson `JsonMapperBuilderCustomizer` (plus a `NON_NULL` default on `Jsons.OBJECT_MAPPER`), so a response never serializes `"field":null` — a failure envelope is `{code,msg}` without `data`.
9. `GlobalExceptionHandler` converts exceptions into unified `Response` payloads. `AtlasException` is the unified framework exception: `new AtlasException("msg")` answers with `code = -90` (`AtlasException.DEFAULT_CODE` = `ErrorCodes.SYSTEM_ERROR`), an explicit `new AtlasException(code, "msg")` keeps its business code; a request that matches no controller or static resource (`NoResourceFoundException` / `NoHandlerFoundException`, e.g. `/favicon.ico`) is answered with an ordinary `404` and no error log; every other exception is offered to the registered `ExceptionResponseResolver` beans (which now return an `ExceptionResponse` carrying the HTTP status) and finally answered with `code = -90` instead of a framework error page.
